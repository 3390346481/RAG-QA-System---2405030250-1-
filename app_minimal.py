...
st.set_page_config(page_title="RAG 本地知识库问答系统", page_icon="🤖", layout="wide")
...
def retrieve_relevant_chunks(query, k=3):
    ...
    for i, similarity in similarities[:k]:
        if similarity > 0:
            results.append({"content": st.session_state.text_chunks[i], "metadata": st.session_state.chunk_metadata[i], "similarity": similarity})
    return results
...
with sidebar:
    st.header("📚 知识库管理")
    uploaded_files = st.file_uploader("上传文档 (PDF/DOCX/TXT)", type=["pdf", "docx", "txt"], accept_multiple_files=True, help="支持 PDF、DOCX 和 TXT 格式")
    if uploaded_files:
                ...
                try:
                    for doc in documents:
                        ...
                        for i, chunk in enumerate(chunks):
                            ...
                            st.session_state.chunk_metadata.append({"source": doc["name"], "chunk": i})
